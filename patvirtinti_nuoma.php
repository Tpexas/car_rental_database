...
if (isset($_GET['auto_id']) && is_numeric($_GET['auto_id'])) {/////////
  ...
  $sql = "SELECT nuomos_kaina FROM automobiliai WHERE auto_id='$auto_id'";
  $result = mysqli_query($conn, $sql);
...
<input  type="disabled" name="nuomos_kaina" value="<?=$kaina['nuomos_kaina']?>"  readonly>
</div>

<div class="mb-3">
  <label>Pasirinkite nuomos pradžią</label>
<input  type="date" name="nuoma_nuo" required>
</div>

<div class="mb-3">
  <label>Pasirinkite planuojamą grąžinimą</label>
<input  type="date" name="nuoma_iki" required>
</div>

<div class="mb-3">
<input class=" btn btn btn-primary" type="submit" id="btnSubmit" name="patvirtinti_n" value="Užsakyti">
</div>
